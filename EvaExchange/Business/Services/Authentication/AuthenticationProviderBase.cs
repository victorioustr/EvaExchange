...
namespace Business.Services.Authentication
{
    ...
    {
        readonly IMobileLoginRepository _logins;
        ...
        public virtual async Task<IDataResult<QTToken>> Verify(VerifyOtpCommand command)
        {
            ...
            if (login == null)
                return new ErrorDataResult<QTToken>(Messages.InvalidCode);
            ...
            return new SuccessDataResult<QTToken>(accessToken, Messages.SuccessfulLogin);
        }

        public abstract Task<LoginUserResult> Login(LoginUserCommand command);

        public abstract Task<QTToken> CreateToken(VerifyOtpCommand command);
