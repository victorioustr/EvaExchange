...
{
    /// <summary>
    /// Provider that logs in using the database.
    /// </summary>
    public class PersonAuthenticationProvider : AuthenticationProviderBase, IAuthenticationProvider
    {
        ...
        public override async Task<QTToken> CreateToken(VerifyOtpCommand command)
        {
            var citizenId = Int64.Parse(command.ExternalUserId);
            ...
            var accessToken = _tokenHelper.CreateToken<QTToken>(user);
